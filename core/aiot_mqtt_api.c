...
static int32_t _core_mqtt_reconnect(core_mqtt_handle_t *mqtt_handle)
{
    ...
    if(mqtt_handle->reconnect_params.backoff_enabled){
        interval_ms = mqtt_handle->reconnect_params.interval_ms * (mqtt_handle->reconnect_params.reconnect_counter + 1) + mqtt_handle->reconnect_params.rand_ms;
    }
...
void *aiot_mqtt_init(void)
{
    ...
    mqtt_handle->reconnect_params.rand_ms = rand_value % CORE_MQTT_DEFAULT_RECONN_RANDLIMIT_MS - CORE_MQTT_DEFAULT_RECONN_RANDLIMIT_MS / 2;
    mqtt_handle->reconnect_params.reconnect_counter = 0;
